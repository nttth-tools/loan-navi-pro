...
import { Bell, Calendar, ChevronRight, AlertTriangle, Clock, TrendingUp, Users, FileCheck, Handshake, ArrowUpRight, Square, ExternalLink, RefreshCw, CheckSquare, CalendarClock } from "lucide-react";
...
function useTodayYMD() {
  ...
  useEffect(() => {
    const d = new Date();
    setText(`${d.getFullYear()}年${d.getMonth()+1}月${d.getDate()}日`);
  }, []);
  return text;
}
...
const buildAlerts = () => [
  { type: "error", icon: AlertTriangle, title: "最終確認日が30日以上前の銀行があります", sub: "7件の銀行が対象です", date: relativeDate(0) },
  { type: "warning", icon: Clock, title: "事前審査の回答期限が近い案件があります", sub: "3件の案件が対象です", date: relativeDate(0) },
  { type: "info", icon: RefreshCw, title: "金利が更新された銀行があります", sub: "住信SBIネット銀行、楽天銀行 など", date: relativeDate(-1) },
  { type: "success", icon: CheckSquare, title: "事前審査が承認されました", sub: "田中 様（住信SBIネット銀行）", date: relativeDate(-1) },
  { type: "success", icon: CheckSquare, title: "新しい銀行が追加されました", sub: "関西みらい銀行", date: relativeDate(-2) },
];
...
const buildRecentCustomers = () => [
  { name: "田中 健一 様", age: 35, amount: "4,000万円", status: "事前審査中", statusColor: "#3b82f6", statusBg: "#DBEAFE", date: relativeDate(0) },
  { name: "佐藤 美咲 様", age: 32, amount: "3,500万円", status: "提案中", statusColor: "#f59e0b", statusBg: "#FEF3C7", date: relativeDate(0) },
  { name: "鈴木 大輔 様", age: 40, amount: "5,000万円", status: "前審査待ち", statusColor: "#f59e0b", statusBg: "#FEF3C7", date: relativeDate(-1) },
  { name: "高橋 優子 様", age: 28, amount: "3,000万円", status: "情報収集中", statusColor: "#64748b", statusBg: "#F1F5F9", date: relativeDate(-2) },
  { name: "伊藤 誠 様", age: 45, amount: "6,000万円", status: "契約准備中", statusColor: "#10b981", statusBg: "#D1FAE5", date: relativeDate(-2) },
];

function buildTasks() {
  const fmtMD = (d: Date) => `${d.getMonth()+1}/${d.getDate()} (${["日","月","火","水","木","金","土"][d.getDay()]})`;
  const d = (n: number) => { const x = new Date(); x.setDate(x.getDate()+n); return x; };
  return [
    { name: "山田 様 書類提出フォロー",    tag: "事前審査", tagColor: "#3b82f6", tagBg: "#DBEAFE", date: `${fmtMD(d(0))} 10:00` },
    { name: "佐藤 様 銀行比較提案書作成",  tag: "提案",     tagColor: "#f59e0b", tagBg: "#FEF3C7", date: `${fmtMD(d(1))} 14:00` },
    { name: "鈴木 様 追加資料依頼",        tag: "事前審査", tagColor: "#3b82f6", tagBg: "#DBEAFE", date: `${fmtMD(d(2))} 11:00` },
    { name: "田中 様 本審査進捗確認",      tag: "本審査",   tagColor: "#8b5cf6", tagBg: "#EDE9FE", date: `${fmtMD(d(2))} 15:00` },
    { name: "高橋 様 面談",               tag: "提案",     tagColor: "#f59e0b", tagBg: "#FEF3C7", date: `${fmtMD(d(3))} 10:00` },
  ];
}
...
export default function DashboardPage() {
  ...
  const staticAlerts  = buildAlerts();
  const recentCustomers = buildRecentCustomers();
  const tasks = buildTasks();

  const loanAlerts = useMemo(() => getAlerts().slice(0, 5), [customers]);

  const alertColorMap = (days: number) => {
    if (days < 0)  return { color: "#EF4444", bg: "#FEF2F2" };
    if (days === 0) return { color: "#F97316", bg: "#FFF7ED" };
    if (days <= 7)  return { color: "#EAB308", bg: "#FEFCE8" };
    if (days <= 14) return { color: "#F59E0B", bg: "#FFFBEB" };
    ...
    <div className="p-4 md:p-7 max-w-[1440px]">
      ...
      <div className="flex items-start justify-between mb-7">
        ...
        <div className="flex items-center gap-3">
          <Link href="/tasks" className="relative p-2 rounded-xl transition-colors hover:bg-gray-50" style={{ border: "1px solid var(--border)" }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_320px] gap-4 mb-4">
        ...
        <Card>
          ...
          <p className="mt-3 text-xs" style={{ color: "var(--text-muted)" }}>※ {todayYMD}時点の情報です。</p>
          <Link href="/rates" className="mt-3 w-full py-2 rounded-xl text-xs font-medium transition-colors hover:opacity-90 flex items-center justify-center" style={{ background: "#DBEAFE", color: "var(--accent-blue)", border: "1px solid #BFDBFE" }}>
            金利ランキングをもっと見る
          </Link>
        </Card>
        ...
        <Card>
          ...
          <p className="mt-3 text-xs" style={{ color: "var(--text-muted)" }}>※ {todayYMD}時点の情報です。</p>
          <Link href="/rates" className="mt-3 w-full py-2 rounded-xl text-xs font-medium transition-colors hover:opacity-90 flex items-center justify-center" style={{ background: "rgba(139,92,246,0.1)", color: "#8b5cf6", border: "1px solid rgba(139,92,246,0.2)" }}>
            固定金利ランキングをもっと見る
          </Link>
        </Card>
        ...
        {/* Alerts */}
        <Card>
          <SectionHeader title="ローンスケジュール アラート" href="/loan-schedule" />
          {loanAlerts.length === 0 ? (
            <>
              <div className="text-center py-4 mb-3">
                <CalendarClock size={28} className="mx-auto mb-2 opacity-30" style={{ color: "var(--text-muted)" }} />
                <p className="text-xs" style={{ color: "var(--text-muted)" }}>期限の近いタスクはありません</p>
              </div>
              <div className="space-y-2">
                {staticAlerts.slice(0, 3).map((a, i) => (
              ...
              {loanAlerts.map((a, i) => {
                ...
                return (
                  <Link key={i} href="/loan-schedule"
                    className="flex gap-2.5 p-2.5 rounded-xl transition-opacity hover:opacity-80"
                    style={{ background: col.bg, textDecoration: "none" }}>
                    <CalendarClock size={14} className="shrink-0 mt-0.5" style={{ color: col.color }} />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium leading-snug" style={{ color: "var(--text-primary)" }}>{a.task.name}</p>
                      <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>{c?.name ?? "—"}</p>
                    </div>
                    <span className="text-xs font-semibold shrink-0 mt-0.5" style={{ color: col.color }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_320px] gap-4">
        ...
        <Card>
          ...
          <Link href="/customers" className="mt-3 w-full py-2 rounded-xl text-xs font-medium transition-colors hover:opacity-90 flex items-center justify-center" style={{ background: "rgba(255,255,255,0.04)", color: "var(--text-secondary)", border: "1px solid var(--border)" }}>
            顧客一覧をもっと見る
          </Link>
        </Card>
        ...
        <Card>
          ...
          <Link href="/tasks" className="mt-3 w-full py-2 rounded-xl text-xs font-medium flex items-center justify-center gap-1.5 transition-colors hover:opacity-90" style={{ background: "#DBEAFE", color: "var(--accent-blue)", border: "1px solid #BFDBFE" }}>
            タスクを追加する +
          </Link>
        </Card>
      </div>
      ...
      <div className="mt-4 rounded-2xl p-5 flex items-center gap-5" style={{ background: "linear-gradient(135deg, #DBEAFE, #EDE9FE)", border: "1px solid #BFDBFE" }}>
        ...
        <Link href="/diagnosis" className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all hover:opacity-90 shrink-0" style={{ background: "linear-gradient(135deg, #3b82f6, #6366f1)", color: "#fff" }}>
